...
class DriverRegisterView(APIView):
    ...
    def post(self, request):
        ...
        if Driver.objects.filter(steam_id=steam_id).exists():
            return Response(
                {'error': 'El steam_id ya está registrado'},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class DriverProfileView(APIView):
    ...
    def get(self, request):
        ...
        try:
            driver = Driver.objects.select_related('team').get(id=request.user.driver_id)
            data = {
                'id': driver.id,
                'steam_id': driver.steam_id,
                'name': driver.name,
